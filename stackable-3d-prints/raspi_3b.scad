...
module raspi_3b() {
    ...
    // Ethernet port
    // Center is 10.25mm from the bottom edge, length is ~21.3mm, width ~16mm, height ~13.5mm.
    // Port typically overhangs PCB edge by ~1.5mm
    color("silver")
    translate([85/2 - 21.3/2 + 1.5, -56/2 + 10.25, 1.1/2 + 13.5/2])
    ...
    // USB port 2 (top)
    // Center is 47.0mm from the bottom edge
    color("silver")
    translate([85/2 - 17.5/2 + 1.5, -56/2 + 47.0, 1.1/2 + 16/2])
        cube([17.5, 14.5, 16], center=true);

    // Power plug (USB-C dimensions as requested)
    ...
    // HDMI port (Full size HDMI for Pi 3B)
    // Center is 32.0mm from the SD-card edge (left edge)
    color("silver")
    translate([-85/2 + 32.0, -56/2 + 11.5/2 - 1.5, 1.1/2 + 6.5/2])
        cube([15.2, 11.5, 6.5], center=true);

    // 3.5mm Audio/Video Jack
    // Center is 53.5mm from the SD-card edge (left edge)
    color("black")
    translate([-85/2 + 53.5, -56/2 + 14.0/2 - 2.0, 1.1/2 + 6.0/2])
